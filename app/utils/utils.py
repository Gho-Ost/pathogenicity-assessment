...
def get_dataset(data_folder, samples, file_type, option_csq=None, options_genotype=None, with_default=False):
    ...
    if with_default:
        ...
        for sample in samples:
            df = pd.read_csv(data_path / f"{sample}_default.csv.gz", sep=";", compression="gzip", low_memory=False).drop("Unnamed: 0", axis=1)
    ...
    genotype_dataframes = []
    for sample in samples:
        df = pd.read_csv(data_path / f"{sample}_genotype.csv.gz", sep=";", compression="gzip", low_memory=False).drop("Unnamed: 0", axis=1)
    ...
    if file_type == "csq" or file_type == "both":
        ...
        for sample in samples:
            df = pd.read_csv(data_path  / f"{sample}_csq.csv.gz", sep=";", compression="gzip", low_memory=False).drop("Unnamed: 0", axis=1)
